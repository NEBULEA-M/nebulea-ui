  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <Component {...getBaseProps()}>
        ...
        >
          {isSelected ? <Gamepad2 className="w-6 h-6 text-green-500" /> : <Sliders className="w-6 h-6 text-blue-500" />}
        </div>
      </Component>
      <p className="text-default-500 select-none font-medium">{isSelected ? "Gamepad Controller" : "Play Console"}</p>
    </div>
  );
};
